...
def render(audio):
    ...
    if os.path.exists(audio.file_path):
        ...
        st.warning(f"⚠️ Audio file not found locally: {audio.file_path}")

    # Compute duration live — the DB column is stored as 0.0 for legacy uploads
    duration = audio.duration if (audio.duration and audio.duration > 0) else get_audio_duration(audio.file_path)
