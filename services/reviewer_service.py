...
def get_annotation_for_task(audio_id: str):
    """Return the SUBMITTED annotation for a given audio task."""
    db = get_db()
    try:
        return (
            db.query(Annotation)
...
def approve(annotation_id: str, reviewer_id: str):
    ...
    try:
        ...
        if all_approved:
            ...
            audio.status = AudioStatus.REVIEWED

            logger.info(
                f"Annotation {annotation_id} FULLY APPROVED by all reviewers "
                f"({sorted(all_reviewer_ids)}). Audio frozen as REVIEWED."
            )
        else:
...
def add_comment(annotation_id: str, reviewer_id: str, comment: str):
    """
    Return an annotation to the annotator with a mandatory feedback comment.
    ...
    try:
        ...
        db.rollback()
        logger.exception(f"Failed to add comment to annotation {annotation_id}")
        return False
