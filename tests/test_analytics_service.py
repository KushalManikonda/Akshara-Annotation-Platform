...
def setup_db():
    ...
    af1 = AudioFile(
        ...
        status=AudioStatus.REVIEWED, uploaded_by="u-admin"
    )
    af2 = AudioFile(
...
class TestKPISummary:
    ...
    def test_approved_duration_and_pct(self):
        ...
        assert data["approved_pct"] == pytest.approx(10.0 / 30.0 * 100, rel=1e-2)
...
class TestAnnotatorLeaderboard:
    ...
    def test_avg_turnaround_present(self):
        data = get_annotator_leaderboard()
        ann1 = next(d for d in data if d["username"] == "annotator1")
        # ann1 has submitted annotations so should have a numeric turnaround
        assert isinstance(ann1["avg_turnaround_days"], float)
